...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Integer[] arr3 = {2, 4, 10};
        Integer[] arr4 = {6, 7, 8};
        Comparable[][] arr = {arr1, arr2, arr3, arr4};
        Comparable[] merge = sortKlist3(arr);
        ...
        Integer[]arr10=new Integer[n];
        for (int i = 0; i <arr10.length ; i++) {
            arr10[i]=random.nextInt(1000);
        }
        Arrays.sort(arr10, new Comparator<Integer>() {
            @Override
...
//region Linked List
       ...
       /* Node<Integer> n1=new Node<>(2);
        ...
        Node<Integer> n8=new Node<>(18);
        Node<Integer> n9=new Node<>(21);
        Node<Integer> n10=new Node<>(23);
        n6.next=n7;
        n7.next=n8;
        n8.next=n9;
        n9.next=n10;
        ...
        Node<Integer> n16=new Node<>(5);
        Node<Integer> n17=new Node<>(10);
        Node<Integer> n18=new Node<>(15);
        Node<Integer> n19=new Node<>(20);
        Node<Integer> n20=new Node<>(25);
        n16.next=n17;
        n17.next=n18;
        n18.next=n19;
        n19.next=n20;
        ...
        head2.next=head3;
        head3.next=head4;
        Node<Integer> merged=mergeLinkedListOfLinkedList(head1);
        printLinkedList(merged);*/
            //endregion

    }

   /* public static Comparable[] sortKlist3(Comparable [][] arr){
        MinHeap minHeap=new MinHeap();
        int counter=arr.length*arr[0].length;
        Comparable[]result=new Comparable[counter];
        int j=0;
        for (int i=0;i<arr.length;i++)
        {
            minHeap.insert(new newNode(arr[i][0],i,0));
        }
        while (counter>0) {
            newNode temp = (newNode) minHeap.extractMin();
            result[j++] = temp.getValue();
            if (temp.getCol()+1<arr[0].length)
            minHeap.insert(new newNode(arr[temp.getRow()][temp.getCol()+1],temp.getRow(),temp.getCol()+1));
            counter--;
        }
        return result;
